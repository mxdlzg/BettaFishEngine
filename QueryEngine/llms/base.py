...
class LLMClient:
    ...
    def _clean_reasoning_content(self, content: str) -> str:
        ...
        重要：这个方法需要区分两种输出类型：
        1. JSON输出 - 需要从文本中提取JSON
        2. Markdown报告 - 需要找到报告开始位置，不能从末尾截断
        """
        if not content:
            return ""
        
        # 首先判断期望的输出类型
        # 如果内容包含Markdown标题（#），则认为是报告格式
        is_markdown_report = bool(re.search(r'^#\s+', content, re.MULTILINE))
        
        if is_markdown_report:
            # Markdown报告处理：找到真正报告的开始位置，保留完整报告
    ...
    def _clean_markdown_output(self, content: str) -> str:
        """
        清理Markdown报告输出 - 找到真正的报告开始位置
        """
        lines = content.split('\n')
        
        # 报告标题特征 - 真正的报告通常以这些关键词开头
        report_indicators = [
            '【', '】', '报告', '分析', '调查', '研究', '舆情',
            '摘要', '概述', '总结', '冲突', '态势', '评估'
        ]
        ...
        garbage_markers = [
            ...
            'This matches', 'The instruction', '*However*', 
            'I will', 'Let me', "Let's write", 'Looking at',
            '*Wait', '*Actually', 'Self-Correction', 'Final check',
            '**Plan', '**Drafting', 'Title:', 'Structure:'
        ]
        ...
        for i, line in enumerate(lines):
            ...
            if stripped.startswith('# ') and not stripped.startswith('## '):
                ...
                # 检查是否是真正的报告标题
                has_indicator = any(kw in title_text for kw in report_indicators)
                has_chinese = any('\u4e00' <= c <= '\u9fff' for c in title_text)
                
                if has_indicator or has_chinese:
                    # 检查后面是否是垃圾内容
                    is_clean = True
